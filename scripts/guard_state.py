...
Two things are guarded:
  ...
  2. Every OTHER non-gitignored file -- ONLY when the CURRENT checkout is the
     PRIMARY one, not a linked task worktree. This also blocks creating a brand-new
     file straight in primary, not just editing an already-tracked one -- new work
     belongs on a worktree too. Real changes belong on a task's dedicated worktree
     (`task.py begin tN`), committed there, landed via `land.py` -- primary should
     stay clean and ready to land at all times. A linked worktree is unaffected:
     that's where coding is supposed to happen. Caught for real: a tracked-doc edit
     landed straight in primary once, self-caught before commit and redone properly
     -- that worked because it was noticed, not because anything stopped it.
     Break-glass for a human: ALLOW_MAIN_EDIT=1.
Gitignored files (CLAUDE.md, .venv, data/, ...) are never guarded by either rule --
no gate applies to them, hand-editing them anywhere is fine.
...
def main() -> int:
    ...
    if _is_gitignored(target):
        return 0  # gitignored local file (e.g. CLAUDE.md) -- no gate applies

    print(
        f"Blocked: {target.relative_to(REPO)} is not gitignored, in the PRIMARY "
        "checkout. Real changes (including new files) go through a task's "
