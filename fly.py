...
from glider_sim import GliderState, Thermal, step   # noqa: E402


def fly_a_circle(start: GliderState, bank_angle, thermal, seconds=120, dt=0.1):
    """Hold a constant bank for `seconds` and record the whole trajectory.

    Returns four NumPy arrays: x, y, z over time, plus the time stamps.
    This is just step() called in a loop -- the simplest possible "policy"
    (do the same thing every tick).
    """
    state = start
    xs, ys, zs, ts = [], [], [], []
    for i in range(int(seconds / dt)):
        state = step(state, bank_angle, thermal, dt)
        xs.append(state.x)
        ys.append(state.y)
        zs.append(state.z)
        ts.append(i * dt)
...
def main():
    ...
    bank = np.radians(40.0)    # 40-degree bank -> a steady circle  # <-- TRY (steeper = tighter + more sink)
    ...
    xa, ya, za, ta = fly_a_circle(far,  bank, thermal)
    xb, yb, zb, tb = fly_a_circle(near, bank, thermal)
